...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.17 2002/12/18 14:34:48 jhnzhang Exp $
\end{verbatim}
...
pWidgets <- list(topRow = list(label1 = label1, entry1 = entry1,
                 ...
                 text1 = text1), radios1 = radios1, checks1 = checks1)
pWidgets
@
...
the interface. Let us click the browse button, selecte a file through
...
lsit box and the radio button and then click the finish button, the
values for \em{entry1}, \em{list1}, and \em{radios1} now become:

<<>>=
value(pWidgets(aWidget)[["topRow"]][["entry1"]])
value(pWidgets(aWidget)[["textRow"]][["list1"]])
value(pWidgets(aWidget)[["radios1"]])
value(pWidgets(aWidget)[["checks1"]])
@
