...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.10 2002/12/13 18:37:12 jhnzhang Exp $
\end{verbatim}
...
list1 <- listBox(name = "list1", value = "Put me in the second row",
                 env = PWEnv)
option <- function(){
    temp <- get("radios1", env = PWEnv)
    value(temp) <- radioOption()
    assign("radios1", temp, env = PWEnv)
}
radios1 <- radioButton(name = c("radio1", "radio2", "radio3"), text =
     c("Radio1", "Radio2", "Radio3"), command = option, env = PWEnv)
