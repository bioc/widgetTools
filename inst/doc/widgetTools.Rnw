...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.14 2002/12/17 14:54:35 jhnzhang Exp $
\end{verbatim}
...
list1 <- listBox(name = "aList", value = c(Option1 = TRUE, Option2 = FALSE,
...
updated by the returned value. Let us click radio2 and then click the
finish button, the values for \em{entry1} and \em{radios1} now become:
