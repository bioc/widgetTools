...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.25 2003/05/06 17:25:34 jhnzhang Exp $
\end{verbatim}
...
with {\texttt{pWidget}} (model) objects being the application data, each of
which contains definitions for a widget element that is going to be
rendered, a {\texttt{widget}} object (controller) that contains a list of
{\texttt{pWidgets}} that will be rendered, and a {\texttt{widgetView}}
(view) that provides the interface to actually render widget elements.

The system is designed in such a way that users only have to deal with
{\texttt{pWidget}} and {\texttt{widget}} objects. {\texttt{widgetView}}
will be managed by {\texttt{widget}}.
...
Then, we can create the {\texttt{pWidgets}} that define the widget elements
...
The following code creates several {\texttt{pWidget}} objects of different types:
...
Notice that the functions defines the behavior of {\texttt{button1}} by
reading a file name using function {\texttt{fileBrowser}} and then assigning
the read in file name to the value of {\texttt{entry1}}. All the data
...
name of the corresponding {\texttt{pWidget}} and to be unique. Values for
{\texttt{list1}}, {\texttt{radios1}}, and {\texttt{checks1}} are named vectors. Names of
...
{\texttt{button1}} specifies that the function will be the command to be
executed when {\texttt{button1}} is pressed. Any button defined is assumed
...
{\texttt{pWidget}} objects have set and get functions to  access and modify
...
appreance of each {\texttt{pWidget}} in the list will determine the layout of the
...
we want to have the first three {\texttt{pWidget}}s namely {\texttt{label1}},
...
another row, we will need to put the {\texttt{pWidgets}} in a list like the
following. Naming of the lists for {\texttt{pWidget}} groups is arbitrary.
...
When we have the list of {\texttt{pWidget}}s ready, we can proceed to create
a {\texttt{widget}} object that will construct a {\texttt{widgetView}} object to
render the widget elements. A {\texttt{widget}} object is defined as following:
...
The following code constructs a {\texttt{widget}} object:
...
words in the text box, and then click the finish button, a {\texttt{widget}}
object will be constructed. The {\texttt{widget}} object contains pWidgets
...
{\texttt{entry1}}, {\texttt{list1}}, {\texttt{text1}}, {\texttt{radios1}}, and {checks1} now become:
