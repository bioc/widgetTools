...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.26 2003/05/20 19:17:08 jhnzhang Exp $
\end{verbatim}
...
of any widget built using \Rpackage{widgetTools} is through the R tcltk package,
...
with \Robject{pWidget} (model) objects being the application data, each of
which contains definitions for a widget element that is going to be
rendered, a \Robject{widget} object (controller) that contains a list of
\Robject{pWidgets} that will be rendered, and a \Robject{widgetView}
...
Then, we can create the \Robject{pWidgets} that define the widget elements
...
Notice that the functions defines the behavior of \Robject{button1} by
reading a file name using function \Robject{fileBrowser} and then assigning
the read in file name to the value of \Robject{entry1}. All the data
...
\Robject{pWidget} objects have set and get functions to  access and modify
...
appreance of each \Robject{pWidget} in the list will determine the layout of the
...
we want to have the first three \Robject{pWidget}s namely {\texttt{label1}},
...
render the widget elements. A \Robject{widget} object is defined as following:
...
words in the text box, and then click the finish button, a \Robject{widget}
object will be constructed. The \Robject{widget} object contains pWidgets
