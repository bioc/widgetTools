...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.12 2002/12/16 18:27:14 jhnzhang Exp $
\end{verbatim}
...
browse2Entry1 <- function(){
    ...
    assign(name(entry1), temp)
#    temp <- get(name(list1), env = PWEnv)
#    value(temp) <- tempValue
#    assign(name(list1), temp, env = PWEnv)
}
...
list1 <- listBox(name = "list1", value = "Put me in the second row",
                 env = PWEnv)
radios1 <- radioButton(name = "radios1", optText = c("radio1", "radio2",
                                        "radio3"), env = PWEnv)
checks1 <- checkButton(name = "checks1", optText = c("check1", "check22",
                                         "check3"), default =
                       "check1", env = PWEnv)
@
