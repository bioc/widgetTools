...
\begin{verbatim}
$Id: widgetTools.Rnw,v 1.11 2002/12/13 22:09:13 jhnzhang Exp $
\end{verbatim}
...
list1 <- listBox(name = "list1", value = "Put me in the second row",
                 env = PWEnv)
radios1 <- radioButton(name = "radios1", text = c("radio1", "radio2",
                                        "radio3"), env = PWEnv)
checks1 <- checkButton(name = "checks1", text = c("check1", "check22",
                                         "check3"), default =
                       "check1", env = PWEnv)
@
